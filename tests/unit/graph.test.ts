...
describe('buildGraph — property-based tests', () => {
      ...
      fc.property(
        ...
        (files) => {
          ...
          for (const f of files) {
            if (!seen.has(f.path)) { seen.add(f.path); uniqueFiles.push(f) }
          }
  ...
  it('Property 2: wiki-link inclusion — a resolved wiki-link produces an edge', () => {
    fc.assert(
      fc.property(
        fc.array(fileEntryArb, { minLength: 2, maxLength: 8 }),
        (files) => {
  ...
  it('Property 3: idempotence — calling buildGraph twice returns same edges', () => {
    fc.assert(
      fc.property(
        fc.array(fileEntryArb, { minLength: 0, maxLength: 8 }),
        (files) => {
          const unique: FileEntry[] = []
          const seen = new Set<string>()
          for (const f of files) {
          ...
          const getAST = (path: string): Root | undefined => {
            ...
            const targets = unique.filter((_, i) => i !== idx).slice(0, 1).map(f => f.name)
            return makeAST(targets)
          }
          const sort = (arr: typeof edges) => [...arr].sort((a, b) => `${a.source}${a.target}`.localeCompare(`${b.source}${b.target}`))
          const edges = buildGraph(unique, getAST)
