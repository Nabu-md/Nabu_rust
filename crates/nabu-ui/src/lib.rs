...
pub fn provide_theme(initial_theme: String) {
    ...
    // user's last chosen theme (dark / light / system). Persisted overrides are
    // read from extra_settings via `settings_get`; when none exists, the
    // `theme` field of the full settings (default "system") is used instead.
    // The first backend write is deferred until this resolves so a transient
    // "dark" write never overwrites a saved "light"/"system" preference.
    let sync_ready = RwSignal::new(false);
    spawn_local(async move {
        let theme_args =
            serde_wasm_bindgen::to_value(&serde_json::json!({ "key": "theme" })).unwrap();
        let result = crate::ipc::tauri_invoke("settings_get", theme_args).await;
