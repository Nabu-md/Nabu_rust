...
//! - `session/request_permission` → returns `Cancelled` (UI-driven permission
...
impl NabuAcpHandler {
    /// Creates a new handler backed by the given storage manager.
    ...
    fn resolve_vault_path(&self, path: &str) -> Result<PathBuf, AcpError> {
        ...
        if raw.is_absolute() {
            ...
            let Ok(canonical_vault) = vault_root.canonicalize() else {
                return Err(AcpError::new(
                    ErrorKind::Internal,
                    format!("Unable to canonicalise vault root: {}", vault_root.display()),
                ));
            };
...
impl AcpClientHandler for NabuAcpHandler {
    ...
    async fn request_permission(
        ...
        // TODO: Wire to the UI permission prompt layer.  For now, deny so the
        // agent doesn't perform unauthorised actions while the prompt UI is
        // not yet integrated.
